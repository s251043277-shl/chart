...
xbar_chart <- qcc(
  ...
  type = "xbar.one",
  title = "X-bar Chart for Part Length (Machine 3, Temp 338, Pres 200)",
  ylab = "Part Length",
  xlab = "Observation"
)
...
p <- plot_ly(plot_data, x = ~x, y = ~y, type = 'scatter', mode = 'lines+markers', name = 'Part Length') %>%
  ...
  layout(title = "X-bar Chart for Part Length (Machine 3, Temp 338, Pres 200)",
...
htmlwidgets::saveWidget(as_widget(p), file.path("/content", "project", "media", "plots", "xbar_one_partlength_machine3.html"), selfcontained = TRUE)
