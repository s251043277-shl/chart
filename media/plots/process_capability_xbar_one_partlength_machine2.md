...
xbar_chart_pc <- qcc(
  ...
  type = "xbar.one",
  title = "Process Capability: X-bar One Chart for Part Length (Machine 2, Temp 338, Pres 200)",
  ylab = "Part Length",
  xlab = "Observation"
)
...
p_pc <- plot_ly(plot_data_pc, x = ~x, y = ~y, type = 'scatter', mode = 'lines+markers', name = 'Part Length') %>%
  ...
  layout(title = "Process Capability: X-bar One Chart for Part Length (Machine 2, Temp 338, Pres 200)",
...
htmlwidgets::saveWidget(as_widget(p_pc), file.path("/content", "project", "media", "plots", "process_capability_xbar_one_partlength_machine2.html"), selfcontained = TRUE)
